...
1. If there are explicit implementations of .equals(), .hashCode(), or .toString() in the data class body or final implementations 
   in a superclass, then these functions are not generated, and the existing implementations are used.

2. If a supertype has .componentN() functions that are open and return compatible types, 
...
A. Data Classes in Kotlin Final by Default:
   ...
   * This design choice ensures that the data class maintains a fixed structure and behavior, 
     which is crucial for maintaining data integrity and predictability.

    Purpose:--
    * Data classes are intended to serve as simple containers for data with automatic implementations of methods such as equals(), hashCode(),
      and toString().
